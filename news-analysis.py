...
class CurrentWallet:
    ...
    def __init__(self, binance_client: BinanceClient, keywords, pairing: str, quantity: float):
        ...
        self.coins_in_hand = {}
        self.coins_in_hand_file_path = 'coins_in_hand.json'

        # use separate files for testnet and live
        if binance_client.is_test_net:
            self.coins_in_hand_file_path = 'testnet_' + self.coins_in_hand_file_path

        # if saved coins_in_hand json file exists then load it
        if os.path.isfile(self.coins_in_hand_file_path):
            with open(self.coins_in_hand_file_path) as file:
                try:
                    self.coins_in_hand = json.load(file)
                except:
                    pass
        ...
        for coin in keywords:
            if coin not in self.coins_in_hand:
    ...
    def save_coins_in_hand_to_file(self):
        # abort saving if dictionary is empty
        if not coins_in_hand:
            return

        # save coins_in_hand to file
        with open(self.coins_in_hand_file_path, 'w') as file:
...
if __name__ == '__main__':
    ...
    for i in count():
        ...
        sentiment_analyzer.current_wallet.save_coins_in_hand_to_file()
